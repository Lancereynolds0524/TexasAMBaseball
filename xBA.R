...
# Some data manipulation is done as well by binning metrics to nearest integer
data <- read.csv("data.csv")
data$ExitSpeed_int <- round(data$ExitSpeed)
data$Angle_int <- round(data$Angle)
data$Direction_int <- round(data$Direction)
data$PitchCall <- toupper(data$PitchCall)
...
xBA <- function(batter_name, df, df_with_ba) {
  ...
  if (total_abs >= 75) {
    xBA_val = round(mean(bb_events$xBA) * (nrow(bb_events) / total_abs), digits = 3)
...
ba_plot + scale_color_gradient(low="blue", high="red") + labs(x = 'Exit Speed', y = 'Launch Angle', title = 'BAcon vs Launch Angle & Exit Speed (2022)') + 
...
xba_plot <- ggplot(data = test_x, aes(x = ExitSpeed_int, y = Angle_int, color = pred_y)) + geom_point()
xba_plot + scale_color_gradient(low="blue", high="red") + labs(x = 'Exit Speed', y = 'Launch Angle', color = 'xBAcon', title = 'xBAcon vs Launch Angle & Exit Speed (2022)') + 
  theme(plot.title = element_text(hjust = 0.5)) + scale_x_continuous(breaks=c(0, 25, 50, 75, 100, 125)) + scale_y_continuous(breaks=c(-75, -50, -25, 0, 25, 50, 75))
...
# Plot xBA (pred_y) vs Launch Angle & Exit Speed
#xba_plot <- ggplot(data = test_x, aes(x = ExitSpeed_int, y = Angle_int, color = pred_y)) + geom_point()
#xba_plot + scale_color_gradient(low="blue", high="red") + labs(x = 'Exit Speed', y = 'Launch Angle', color = 'xBA', title = 'xBA vs Launch Angle & Exit Speed (2022)') + 
#  theme(plot.title = element_text(hjust = 0.5)) + scale_x_continuous(breaks=c(0, 25, 50, 75, 100, 125)) + scale_y_continuous(breaks=c(-75, -50, -25, 0, 25, 50, 75))
